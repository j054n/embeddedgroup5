...
#pragma udata msgqueue5
static msg_queue mwReadQueue;
signed char	MWQueue_sendmsg(unsigned char length,unsigned char msgtype,void *data)
{/*
#ifdef DEBUG
	if (! in_main()) {
		return(MSG_NOT_IN_MAIN);
...
#endif*/
	return(send_msg(&mwReadQueue,length,msgtype,data));
}

signed char	MWQueue_recvmsg(unsigned char maxlength,unsigned char *msgtype,void *data)
{/*
#ifdef DEBUG
	if (!in_high_int()) {
		return(MSG_NOT_IN_HIGH);
...
#endif*/
	return(recv_msg(&mwReadQueue,maxlength,msgtype,data));
}
